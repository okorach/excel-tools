...
Sub ImportToCalculator(accName As String)
    ...
    With Sheets(INTEREST_CALC_SHEET)
        ...
        '.ListObjects(2).ListColumns(4).DataBodyRange.Cells(1).formula = "=IF([Nbr de p�riodes]<=0;IF(OR([Date]>=target_date;[Date]<=start_date);0;[Montant]);[Montant]*(1+$R$1)^[Nbr de p�riodes])"
